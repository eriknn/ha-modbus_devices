####################################################
##### Swegon Conductor Apartment W3 Modbus     #####
##### Based on "conductor apartment re modbus.pdf" #
##### Register set W3 (with airflow controls)  #####
####################################################
...
GROUP_ALARMS_HW = ModbusGroup(ModbusMode.DISCRETE_INPUTS, ModbusPollMode.POLL_ON)
...
GROUP_AIRFLOW = ModbusGroup(ModbusMode.HOLDING, ModbusPollMode.POLL_ON)
...
class Device(ModbusDevice):
    ...
    def loadDatapoints(self):
        ...
        # Alarms that reset automatically (1x0006-1x0054)
        self.Datapoints[GROUP_ALARMS] = {
            ...
            # More auto-reset alarms (1x0047-1x0054)
        ...
        # Addresses are 0-indexed (3x0001 = address 0)
        # ============================================
        ...
        self.Datapoints[GROUP_SENSORS] = {
            ...
            "Flow Setpoint RU": ModbusDatapoint(
                address=31,
                entity_data=EntityDataSensor(
                    units=PERCENTAGE,
                    icon="mdi:fan"
        ...
        self.Datapoints[GROUP_OUTPUTS] = {
            ...
            "Supply Air Damper Position": ModbusDatapoint(address=54, entity_data=EntityDataSensor(units=PERCENTAGE, icon="mdi:air-filter")),
            "Exhaust Air Damper Position": ModbusDatapoint(address=55, entity_data=EntityDataSensor(units=PERCENTAGE, icon="mdi:air-filter")),
        }

        # ============================================
        # HOLDING REGISTERS (4x) - Read/Write
        # Addresses are 0-indexed (4x0001 = address 0)
        # ============================================
        ...
        # Airflow setpoints
        self.Datapoints[GROUP_AIRFLOW] = {
            "Normal Flow SA": ModbusDatapoint(address=35, entity_data=EntityDataNumber(units=PERCENTAGE, min_value=0, max_value=100, step=1, icon="mdi:fan")),
            "Normal Flow EA": ModbusDatapoint(address=36, entity_data=EntityDataNumber(units=PERCENTAGE, min_value=0, max_value=100, step=1, icon="mdi:fan")),
            "Economy Flow SA": ModbusDatapoint(address=37, entity_data=EntityDataNumber(units=PERCENTAGE, min_value=0, max_value=100, step=1, icon="mdi:fan")),
            "Economy Flow EA": ModbusDatapoint(address=38, entity_data=EntityDataNumber(units=PERCENTAGE, min_value=0, max_value=100, step=1, icon="mdi:fan")),
            "Night Cool Flow SA": ModbusDatapoint(address=39, entity_data=EntityDataNumber(units=PERCENTAGE, min_value=50, max_value=100, step=1, icon="mdi:fan")),
            "Night Cool Flow EA": ModbusDatapoint(address=40, entity_data=EntityDataNumber(units=PERCENTAGE, min_value=50, max_value=100, step=1, icon="mdi:fan")),
            "Boost Flow SA": ModbusDatapoint(address=41, entity_data=EntityDataNumber(units=PERCENTAGE, min_value=0, max_value=100, step=1, icon="mdi:fan")),
            "Boost Flow EA": ModbusDatapoint(address=42, entity_data=EntityDataNumber(units=PERCENTAGE, min_value=0, max_value=100, step=1, icon="mdi:fan")),
            "Stand-by Flow SA": ModbusDatapoint(address=45, entity_data=EntityDataNumber(units=PERCENTAGE, min_value=0, max_value=100, step=1, icon="mdi:fan")),
            "Stand-by Flow EA": ModbusDatapoint(address=46, entity_data=EntityDataNumber(units=PERCENTAGE, min_value=0, max_value=100, step=1, icon="mdi:fan")),
            "Manual SA Flow": ModbusDatapoint(address=63, entity_data=EntityDataNumber(units=PERCENTAGE, min_value=0, max_value=100, step=1, icon="mdi:fan")),
            "Manual EA Flow": ModbusDatapoint(address=64, entity_data=EntityDataNumber(units=PERCENTAGE, min_value=0, max_value=100, step=1, icon="mdi:fan")),
        }

        # Configuration settings (accessible via CONFIG group)
        self.Datapoints[ModbusDefaultGroups.CONFIG] = {
            ...
            "Room Unit Min Air Flow": ModbusDatapoint(
                address=30,
                entity_data=EntityDataNumber(
                    units=PERCENTAGE,
                    min_value=5,
            ...
            "Room Unit Max Air Flow": ModbusDatapoint(
                address=31,
                entity_data=EntityDataNumber(
                    units=PERCENTAGE,
                    min_value=50,
            ...
            "Boost Temp Positive Hysteresis": ModbusDatapoint(
                address=32,
                entity_data=EntityDataNumber(
                    units=UnitOfTemperature.CELSIUS,
                    min_value=1,
            ...
            "Boost Temp Negative Hysteresis": ModbusDatapoint(
                address=33,
                entity_data=EntityDataNumber(
                    units=UnitOfTemperature.CELSIUS,
                    min_value=0,
    ...
    def onAfterFirstRead(self):
        ...
        # Update model name from component name
    ...
    def onAfterRead(self):
        ...
        # Aggregate alarms into attributes on the Active Alarms sensor
        alarms = self.Datapoints[GROUP_ALARMS]
        attrs = {}

        # Check all alarm datapoints (skip the summary ones)
        ...
        # Update Active Alarms entity attributes
        if "Active Alarms" in alarms and alarms["Active Alarms"].entity_data:
            alarms["Active Alarms"].entity_data.attrs = attrs
            # Invert the "No Active Alarms" value for the binary sensor
            # In the PDF, 1 = no alarms, 0 = has alarms
            no_active_alarms = alarms.get("No Active Alarms")
            if no_active_alarms:
                # Set the Active Alarms binary sensor: True when there ARE alarms
                alarms["Active Alarms"].value = not no_active_alarms.value

        # Set current alarms summary for UI
